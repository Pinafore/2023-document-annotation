...
'''
def get_coherence(keywords, corpus, type):
...
def create_ETM_and_save(num_topics, iters, dataset, train_length):
    ...
    coherence_cv = get_coherence(topics, processed_data, 'c_v')
    coherence_umass = get_coherence(topics, processed_data, 'u_mass')
...
def create_Bertopic_model(num_topics, iters, dataset, train_length, user_labels):
    ...
    '''
    topic_model = BERTopic(nr_topics=16, top_n_words = 30, calculate_probabilities=True, verbose=True, embedding_model='all-MiniLM-L6-v2')
    ...
    topic_model.save('./Model/Bertopic_instance_{}.pkl'.format(num_topics))
...
def create_CTM_Model(num_topics, iters, dataset, train_length, raw_path):
    ...
    coherence_cv = get_coherence(model_topics, processed_data, 'c_v')
    coherence_umass = get_coherence(model_topics, processed_data, 'u_mass')
    print('cv coherence: ', coherence_cv)
    print('umass coherence: ', coherence_umass)
...
def create_sCTM_Model(num_topics, iters, dataset, train_length, raw_path, user_labels):
    save_model_path = './Model/SCTM_{}.pkl'.format(num_topics)
    ...
    '''
    qt = TopicModelDataPreparation("paraphrase-distilroberta-base-v2")
    training_dataset = qt.fit(text_for_contextual=raw_texts, text_for_bow=documents, labels=labels)

    '''
    Initialize and Train the CTM model
    '''
    ctm = CombinedTM(bow_size=len(qt.vocab), contextual_size=768, n_components=num_topics, num_epochs = iters, label_size=len(set(labels)))
    ctm.fit(training_dataset)
...
def main():
    ...
    argparser.add_argument("--model_type", help="type of the model, SLDA, LDA, ETM, CTM, PLDA, LLDA, Bertopic",
    ...
    elif args.model_type == 'SCTM':
        df = pd.read_json(args.raw_text_path)
        sub_labels = df.sub_labels.values.tolist()
        sctm_labels = {i: sub_labels[i] for i in range(len(sub_labels))}

        create_sCTM_Model(args.num_topics, args.num_iters, args.load_data_path, full_len, args.raw_text_path, sctm_labels)
