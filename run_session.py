    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        if row is not None:
            cursor.execute('''INSERT INTO recommendations
                            (user_id, label, doc_id, response_time, actual_label, local_training_acc, local_testing_acc, global_training_acc, global_testing_acc, topic_information)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
...
for j in range(0, 20):
    ...
    for i in range(len(true_labels)):
        recommend_result = nist_recommend(user_id, true_labels[int(recommend_id)], recommend_id, '0')
